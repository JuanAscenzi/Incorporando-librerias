...
//renderizador con forEach pude poner ahora un const con id,imagen,etc para luego unicamente llamrlo por su nombrecreo que se puede hacer mas lindo agregandole cosas pero no se casi nada de css
canillas.forEach(canillas => {
    const {id,nombre,precio,imagen} = canillas
    let productoRenderizado = document.createElement("div")
    productoRenderizado.innerHTML = `
        <div class="card" style="width: 18rem;">
        <div class="text-center">
        <img src="${imagen}" class="rounded" alt="">
        </div>
        <div class="card-body">
...
boton.addEventListener("click", () => comprar(canillas) && Swal.fire("Compraste ${canilla.nombre}"))
})
...
const comprar = (canillas) => {
    ...
    productoComprado === undefined ? carrito.push({...canillas, cantidad: 1}) : 
